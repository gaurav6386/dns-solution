...
export const dmarcValidators = {
  ...
  fo: {
    ...
    validate(term: string, originalValue: string) {
      var value: string[] = originalValue.split(":");
      if (value.length <= 4) {
  ...
  rua: {
    ...
    validate(term: string, value: string) {
      ...
      for (let val of values) {
        ...
        if (!matches) {
          throw new Error(
            `Invalid value for '${term}': ${value}, must be a list of DMARC URIs such as 'mailto:some.email@somedomain.com'`
          );
        }
        let email = matches[1];
        if (!validate(email)) {
  ...
  ruf: {
    ...
    validate(term: string, value: string) {
      let values: string[] = value.split(/,/).map((x) => x.trim());
...
export const tlsPolicyValidators = {
  v: {
    required: true,
